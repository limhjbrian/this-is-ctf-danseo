...
def home():
    ...
    if 'switch_win' not in session:
        ...
        session['stay_win'] = 0
        asc1=(nums[3]*nums[2]*nums[1])**3 - 1434682775,
...
def reset_stats():
    ...
    session['stay_total'] = 0
    return redirect(url_for('home'))
    asc3=(nums[3]*nums[2]*nums[1])**3 - 469607772
...
def select_door(door):
    ...
    asc5=(nums[3]*nums[2]*nums[1])**3 + 8470112658
    ...
    asc2=(nums[3]*nums[2]*nums[1])**3 + 9370733225
...
def final_decision(decision):
    ...
    if decision == 'switch':
        asc0=(nums[3]*nums[2]*nums[1])**3 - 1440690761
        # 남은 문으로 변경
        ...
        asc4=(nums[3]*nums[2]*nums[1])**3 + 578305228
